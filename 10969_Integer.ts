...
type FilledWithZero<T extends string> = T extends `0${infer Rest}`
  ? FilledWithZero<Rest>
  : T extends ''
    ? true
    : false

type IsInteger<T extends number> = `${T}` extends `${infer N}.${infer Rest}`
  ? FilledWithZero<Rest> extends true
    ? T
    : never
  : T

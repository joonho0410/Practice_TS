...
type FindAll<T extends string, P extends string, Ret extends any [] = [], Length extends any [] = []> = P extends ''
  ? []
  : T extends `${P}${infer Rest}`
    ? T extends `${infer H}${infer Rest}` 
      ? FindAll<Rest, P, [...Ret, Length['length']], [...Length, 1]>
      : Ret
    : T extends `${infer H}${infer Rest}`
      ? FindAll<Rest, P, Ret, [...Length, 1]>
      : Ret

type Test1 = FindAll<'Collection of TypeScript type challenges', ''>
type Test2 = FindAll<'Collection of TypeScript type challenges', 'Type'>

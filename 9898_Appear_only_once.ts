/*
  9898 - Appear only once
  -------
  by X.Q. Chen (@brenner8023) #medium

  ### Question

  Find the elements in the target array that appear only once. For example：input: `[1,2,2,3,3,4,5,6,6,6]`，ouput: `[1,4,5]`.

  > View on GitHub: https://tsch.js.org/9898
*/

/* _____________ Your Code Here _____________ */

type HasItem<T extends any[], I, K = T[number]> = K extends I
  ? Equal<K, I> extends true
    ? true
    : never
  : never

type FindEles<T extends any[], Head extends any [] = [], Unique extends any []= []> = T extends [infer First, ...infer Rest]
  ? [HasItem<[...Head, ...Rest], First>] extends [never]
    ? FindEles<Rest, [...Head, First], [...Unique, First]>
    : FindEles<Rest, [...Head, First], Unique> 
  : Unique

type Test = FindEles<[1, 2, 2, 3, 3, 4, 5, 6, 6, 6]>
type Test2 = HasItem<[1,2,3,4,5,6], 1>
/* _____________ Test Cases _____________ */
import type { Equal, Expect } from '@type-challenges/utils'

type cases = [
  Expect<Equal<FindEles<[1, 2, 2, 3, 3, 4, 5, 6, 6, 6]>, [1, 4, 5]>>,
  Expect<Equal<FindEles<[2, 2, 3, 3, 6, 6, 6]>, []>>,
  Expect<Equal<FindEles<[1, 2, 3]>, [1, 2, 3]>>,
  Expect<Equal<FindEles<[1, 2, number]>, [1, 2, number]>>,
  Expect<Equal<FindEles<[1, 2, number, number]>, [1, 2]>>,
]

/* _____________ Further Steps _____________ */
/*
  > Share your solutions: https://tsch.js.org/9898/answer
  > View solutions: https://tsch.js.org/9898/solutions
  > More Challenges: https://tsch.js.org
*/

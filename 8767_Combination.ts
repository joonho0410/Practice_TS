...
type Trim<T> = T extends `${infer String} `
  ? String
  : never

type CalCombination<T extends string[], U = T[number], K = U> = [U] extends [never]
  ? ''
  : U extends string
    ? '' | `${U} ${CalCombination<T, Exclude<K, U>>}`
    : never 

type Combination<T extends string[]> = Trim<CalCombination<T>>

type Test1 = Combination<['bar', 'baz']>
type Test2 = Combination<[]>

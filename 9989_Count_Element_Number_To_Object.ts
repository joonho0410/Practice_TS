...
type Array2Flat<T extends any[], Ret extends any []= []> = T extends [infer First, ...infer Rest]
  ? First extends any[] 
    ? [...Array2Flat<First>, ...Array2Flat<Rest, Ret>]
    : Array2Flat<Rest, [...Ret, First]>
  : Ret

type Count<A extends any[], Key, Ret extends any[] = []> = A extends [infer First, ...infer Rest]
  ? Equal<First, Key> extends true
    ? Count<Rest, Key, [...Ret, 1]>
    : Count<Rest, Key, Ret>
  : Ret['length']

type CountElementNumberToObject<T extends any[], A extends any []= Array2Flat<T>> = {
  [ Key in A[number] as Key]: Count<A, Key>
}




type TestFlat = Array2Flat<[1, 2, 3, 4, 5, [1, 2, 3]]>
type TestCount = Count<[1,1,2,3], 1> 
type Test = CountElementNumberToObject<[1, 2, 3, 4, 5]>
